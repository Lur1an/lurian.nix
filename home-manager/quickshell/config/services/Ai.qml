...
3. Start chatting!`;
    ...
    Process {
        id: cacheDirectoryCreator
        command: ["mkdir", "-p", "/home/lurian/.cache/quickshell-ai"]
        onExited: {
            // Load saved config after ensuring directory exists
            root.loadConfig();
        }
    }

    Component.onCompleted: {
        // Create cache directory for config storage, then load config
        cacheDirectoryCreator.running = true;
    }
}
