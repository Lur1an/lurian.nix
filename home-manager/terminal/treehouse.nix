    ...
    text = ''
      ...
      if [[ -z $worktree ]]; then
        if [[ -f $main_repo/treehouse.toml ]] && grep -Eq '^[[:space:]]*root[[:space:]]*=' "$main_repo/treehouse.toml"; then
          printf 'Remove the repo-level root from %s; treehouseConfig.root controls worktree storage.\n' \
...
in {
  options.treehouseConfig = {
    enable = lib.mkEnableOption "Treehouse worktree management";

    root = lib.mkOption {
      type = lib.types.str;
      default = "${config.home.homeDirectory}/.treehouse";
      description = "Absolute directory used for Treehouse worktree pools";
    };

    maxTrees = lib.mkOption {
      type = lib.types.ints.positive;
      default = 16;
      description = "Maximum number of worktrees in each Treehouse pool";
    };
  };
    ...
    assertions = [
      {
        ...
        message = "treehouseConfig.root must be an absolute path";
      }
    ];
