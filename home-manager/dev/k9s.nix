{config, ...}: let
  colors = config.colorscheme.palette;
in {
  programs.k9s = {
    enable = true;
    skins = {
      echidna = {
        k9s = {
          body = {
            fgColor = "#a8a8a8";
            bgColor = "#141216";
            logoColor = "#7ceec8";
          };
          prompt = {
            fgColor = "#a8a8a8";
            bgColor = "#110f13";
            suggestColor = "#8daeef";
          };
          help = {
            fgColor = "#a8a8a8";
            bgColor = "#141216";
            sectionColor = "#7ceec8";
            keyColor = "#8daeef";
            numKeyColor = "#E356A7";
          };
          frame = {
            title = {
              fgColor = "#7ceec8";
              bgColor = "#141216";
              highlightColor = "#EFA554";
              counterColor = "#f4fa8c";
              filterColor = "#7ceec8";
            };
            border = {
              fgColor = "#8d58ec";
              focusColor = "#ECECEC";
            };
            menu = {
              fgColor = "#a8a8a8";
              keyColor = "#8daeef";
              numKeyColor = "#E356A7";
            };
            crumbs = {
              fgColor = "#141216";
              bgColor = "#E356A7";
              activeColor = "#f4fa8c";
            };
            status = {
              newColor = "#8daeef";
              modifyColor = "#ECECEC";
              addColor = "#7ceec8";
              pendingColor = "#EFA554";
              errorColor = "#E356A7";
              highlightColor = "#8d58ec";
              killColor = "#BD93F9";
              completedColor = "#505A6D";
            };
          };
          info = {
            fgColor = "#EFA554";
            sectionColor = "#a8a8a8";
          };
          views = {
            table = {
              fgColor = "#a8a8a8";
              bgColor = "#141216";
              cursorFgColor = "#1a171c";
              cursorBgColor = "#274540";
              markColor = "#f4fa8c";
              header = {
                fgColor = "#f4fa8c";
                bgColor = "#141216";
                sorterColor = "#8d58ec";
              };
            };
            xray = {
              fgColor = "#a8a8a8";
              bgColor = "#141216";
              cursorColor = "#274540";
              cursorTextColor = "#141216";
              graphicColor = "#EFA554";
            };
            charts = {
              bgColor = "#141216";
              chartBgColor = "#141216";
              dialBgColor = "#141216";
              defaultDialColors = [
                "#7ceec8"
                "#E356A7"
              ];
              defaultChartColors = [
                "#7ceec8"
                "#E356A7"
              ];
              resourceColors = {
                cpu = [
                  "#8d58ec"
                  "#8daeef"
                ];
                mem = [
                  "#f4fa8c"
                  "#EFA554"
                ];
              };
            };
            yaml = {
              keyColor = "#8daeef";
              valueColor = "#a8a8a8";
              colonColor = "#505A6D";
            };
            logs = {
              fgColor = "#a8a8a8";
              bgColor = "#141216";
              indicator = {
                fgColor = "#ECECEC";
                bgColor = "#141216";
                toggleOnColor = "#7ceec8";
                toggleOffColor = "#505A6D";
              };
            };
          };
          dialog = {
            fgColor = "#f4fa8c";
            bgColor = "#505A6D";
            buttonFgColor = "#141216";
            buttonBgColor = "#818181";
            buttonFocusFgColor = "#141216";
            buttonFocusBgColor = "#EFA554";
            labelFgColor = "#f4fa8c";
            fieldFgColor = "#a8a8a8";
          };
        };
      };
      catpuccin-mocha = {
        k9s = {
          body = {
            fgColor = "#cdd6f4";
            bgColor = "#1e1e2e";
            logoColor = "#cba6f7";
          };
          prompt = {
            fgColor = "#cdd6f4";
            bgColor = "#181825";
            suggestColor = "#89b4fa";
          };
          help = {
            fgColor = "#cdd6f4";
            bgColor = "#1e1e2e";
            sectionColor = "#a6e3a1";
            keyColor = "#89b4fa";
            numKeyColor = "#eba0ac";
          };
          frame = {
            title = {
              fgColor = "#94e2d5";
              bgColor = "#1e1e2e";
              highlightColor = "#f5c2e7";
              counterColor = "#f9e2af";
              filterColor = "#a6e3a1";
            };
            border = {
              fgColor = "#cba6f7";
              focusColor = "#b4befe";
            };
            menu = {
              fgColor = "#cdd6f4";
              keyColor = "#89b4fa";
              numKeyColor = "#eba0ac";
            };
            crumbs = {
              fgColor = "#1e1e2e";
              bgColor = "#eba0ac";
              activeColor = "#f2cdcd";
            };
            status = {
              newColor = "#89b4fa";
              modifyColor = "#b4befe";
              addColor = "#a6e3a1";
              pendingColor = "#fab387";
              errorColor = "#f38ba8";
              highlightColor = "#89dceb";
              killColor = "#cba6f7";
              completedColor = "#6c7086";
            };
          };
          info = {
            fgColor = "#fab387";
            sectionColor = "#cdd6f4";
          };
          views = {
            table = {
              fgColor = "#cdd6f4";
              bgColor = "#1e1e2e";
              cursorFgColor = "#313244";
              cursorBgColor = "#45475a";
              markColor = "#f5e0dc";
              header = {
                fgColor = "#f9e2af";
                bgColor = "#1e1e2e";
                sorterColor = "#89dceb";
              };
            };
            xray = {
              fgColor = "#cdd6f4";
              bgColor = "#1e1e2e";
              cursorColor = "#45475a";
              cursorTextColor = "#1e1e2e";
              graphicColor = "#f5c2e7";
            };
            charts = {
              bgColor = "#1e1e2e";
              chartBgColor = "#1e1e2e";
              dialBgColor = "#1e1e2e";
              defaultDialColors = [
                "#a6e3a1"
                "#f38ba8"
              ];
              defaultChartColors = [
                "#a6e3a1"
                "#f38ba8"
              ];
              resourceColors = {
                cpu = [
                  "#cba6f7"
                  "#89b4fa"
                ];
                mem = [
                  "#f9e2af"
                  "#fab387"
                ];
              };
            };
            yaml = {
              keyColor = "#89b4fa";
              valueColor = "#cdd6f4";
              colonColor = "#a6adc8";
            };
            logs = {
              fgColor = "#cdd6f4";
              bgColor = "#1e1e2e";
              indicator = {
                fgColor = "#b4befe";
                bgColor = "#1e1e2e";
                toggleOnColor = "#a6e3a1";
                toggleOffColor = "#a6adc8";
              };
            };
          };
          dialog = {
            fgColor = "#f9e2af";
            bgColor = "#9399b2";
            buttonFgColor = "#1e1e2e";
            buttonBgColor = "#7f849c";
            buttonFocusFgColor = "#1e1e2e";
            buttonFocusBgColor = "#f5c2e7";
            labelFgColor = "#f5e0dc";
            fieldFgColor = "#cdd6f4";
          };
        };
      };
    };
    settings = {
      k9s = {
        ui = {
          skin = "catpuccin-mocha";
        };
        refreshRate = "2s";
        readOnly = true;
        logoless = true;
      };
    };
  };
}
