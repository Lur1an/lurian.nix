...
function M.pre()
    vim.g.mapleader = " "
    if features.nvchad then require("nixvim.nvchad").pre() end
    if features.treesitter then vim.treesitter.language.register("markdown", "mdx") end
end

function M.post()
    if features.nvchad then require("nixvim.nvchad").post() end
    ...
    if features.dap and features.dap_ui then
        local dap, dapui = require("dap"), require("dapui")
        dap.listeners.after.event_initialized["dapui_config"] = function() dapui.open() end
        dap.listeners.before.event_terminated["dapui_config"] = function() dapui.close() end
        dap.listeners.before.event_exited["dapui_config"] = function() dapui.close() end
    end
    if features.opencode and features.snacks then
    ...
    if features.treesitter then require("configs.verus-treesitter").setup() end
end
