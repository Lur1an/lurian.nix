...
function M.watch_command(opts)
	local command = vim.trim(opts.args)
	local interval = tonumber(opts.fargs[#opts.fargs]) or 2
	...
	local interval_ms = math.max(1, math.floor(interval * 1000))
	watch_timers[buf] = timer
	timer:start(0, interval_ms, vim.schedule_wrap(run_command))
